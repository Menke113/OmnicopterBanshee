...
thrust_desired_x = rng.random() * 3000 * 0.009806652 # get these from from the dynamics function
torque_desired_x = rng.random() * 300 * 0.009806652
thrust_desired_y = rng.random() * 3000 * 0.009806652
torque_desired_y = rng.random() * 300 * 0.009806652
thrust_desired_z = rng.random() * 3000 * 0.009806652
torque_desired_z = rng.random() * 300 * 0.009806652
...
def torque_constraint_z(x): # torque from just the thrust forces, not including moments from the props
    alpha = (x - xp)/timestep
    return - (I * alpha[0]) - (I * alpha[1]) - (I * alpha[2]) - (I * alpha[3])
