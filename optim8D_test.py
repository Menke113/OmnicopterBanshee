...
I = 2.03e-14 * 0.009806652  # propeller + motor bell moment of inertia, in g * mm^2 to Newtons * mm^2
...
def objective(x): # x is new 8D vector of w's, xp is prev 8D vector of w's
...
def torque_constraint_z(x): # torque from just the thrust forces, not including moments from the props
    return - (I * ((x[0] - xp[0])/timestep)) - (I * ((x[1] - xp[1])/timestep)) - (I * ((x[2] - xp[2])/timestep)) - (I * ((x[3] - xp[3])/timestep)) - torque_desired_z
...
def torque_constraint_motor1(x):
    return max_torque_motor_2306 - (I * ((x[0] - xp[0])/timestep))

def torque_constraint_motor2(x):
    return max_torque_motor_2306 - (I * ((x[1] - xp[1])/timestep))

def torque_constraint_motor3(x):
    return max_torque_motor_2306 - (I * ((x[2] - xp[2])/timestep))

def torque_constraint_motor4(x):
    return max_torque_motor_2306 - (I * ((x[3] - xp[3])/timestep))

def torque_constraint_motor5(x):
    return max_torque_motor_2806 - (I * ((x[4] - xp[4])/timestep))

def torque_constraint_motor6(x):
    return max_torque_motor_2806 - (I * ((x[5] - xp[5])/timestep))

def torque_constraint_motor7(x):
    return max_torque_motor_2806 - (I * ((x[6] - xp[6])/timestep))

def torque_constraint_motor8(x):
    return max_torque_motor_2806 - (I * ((x[7] - xp[7])/timestep))
...
constraints = [{'type':'eq', 'fun':lambda x: ((k_t * x[4]**2) * np.sign(x[4])) + ((k_t * x[5]**2) * np.sign(x[5])) - thrust_desired_x},
               
               # thrust constraint in y axis:
               {'type':'eq', 'fun':lambda x: ((k_t * x[6]**2) * np.sign(x[6])) + ((k_t * x[7]**2) * np.sign(x[7])) - thrust_desired_y},

               # thrust constraint in z axis:
               {'type':'eq', 'fun':lambda x: ((k_t * x[0]**2) * np.sign(x[0])) + ((k_t * x[1]**2) * np.sign(x[1])) + ((k_t * x[2]**2) * np.sign(x[2])) + ((k_t * x[3]**2) * np.sign(x[3])) - thrust_desired_z},
               ...
               {'type':'eq', 'fun':lambda x: - (I * ((x[0] - xp[0])/timestep)) - (I * ((x[1] - xp[1])/timestep)) - (I * ((x[2] - xp[2])/timestep)) - (I * ((x[3] - xp[3])/timestep)) - torque_desired_z},
...
sol = minimize(lambda x: np.abs(k_e * np.sum(x) + k_r * (np.sum(x - xp))), xp, method='trust-constr', bounds=bnds, constraints=constraints, options={'gtol': 10, 'maxiter' : 10000}) #, jac = '2-point', hess = hess)
